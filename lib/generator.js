   ...
   Meteor.methods({
      'generateMenu': function(user) {
         ...
         while (target) {
            ...
            let options = db_nut_data.find({ Nutr_No: target }, { sort:{Nutr_Val:-1}, limit:5 }).fetch();

            // Pick (eventually) a random one and collect other data
            let choice = Food(options[0].NDB_No);

            for (i = 0; i < nutrients.length; ++i) {
               currValues[nutrients[i]] += Number(choice.nutrients[nutrients[i]]) || 0;
            }
            foods.push(choice);
